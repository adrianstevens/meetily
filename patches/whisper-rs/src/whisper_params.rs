...
impl<'a, 'b> FullParams<'a, 'b> {
    ...
    pub fn new(sampling_strategy: SamplingStrategy) -> FullParams<'a, 'b> {
        ...
        match sampling_strategy {
            SamplingStrategy::Greedy { best_of: _ } => {
                // greedy.best_of not available in Windows bindings (opaque struct)
            }
            SamplingStrategy::BeamSearch {
                beam_size: _,
                patience: _,
            } => {
                // beam_search fields not available in Windows bindings (opaque struct)
            }
        }
